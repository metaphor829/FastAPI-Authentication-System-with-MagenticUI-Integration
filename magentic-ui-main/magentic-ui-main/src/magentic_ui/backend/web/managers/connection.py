...
class WebSocketManager:
    ...
    def _parse_api_error(self, error: Exception) -> tuple[str, str]:
        """
        Parse API error and return user-friendly message and stop reason

        Args:
            error (Exception): Exception that occurred

        Returns:
            tuple[str, str]: (user_message, stop_reason)
        """
        ...
        if 'insufficient credits' in error_str or '402' in error_str:
            return (
                "🚫 API余额不足 - 您的OpenRouter账户余额不足，请充值或配置自己的API密钥",
                "API余额不足"
            )

        # API密钥无效
        if any(phrase in error_str for phrase in ['invalid api key', 'unauthorized', '401']):
            return (
                "🔑 API密钥无效 - 请检查并更新您的API密钥",
                "API密钥无效"
            )
        ...
        if any(phrase in error_str for phrase in ['rate limit', 'too many requests', '429']):
            return (
                "⏱️ 请求过于频繁 - 请稍后再试或升级API计划",
                "API速率限制"
            )

        # 网络错误
        if any(phrase in error_str for phrase in ['connection', 'network', 'timeout']):
            return (
                "🌐 网络连接错误 - 请检查网络连接或稍后重试",
                "网络连接错误"
            )
        ...
        # 配额用完
        if any(phrase in error_str for phrase in ['quota', 'exceeded']):
            return (
                "📊 API配额已用完 - 请检查账户余额或等待配额重置",
                "API配额用完"
            )
        ...
        return (original_error, "处理过程中发生错误")
    ...
    async def _handle_stream_error(self, run_id: int, error: Exception) -> None:
        ...
        if run_id not in self._closed_connections:
            # 解析错误获取用户友好的消息
            user_message, stop_reason = self._parse_api_error(error)

            error_result = TeamResult(
                task_result=TaskResult(
                    messages=[TextMessage(source="system", content=user_message)],
            ...
            await self._update_run(
                run_id, RunStatus.ERROR, team_result=error_result, error=user_message
            )
